...
G <- cbind(0, (b3+3*b4)/log(b2+2), log(b2+2), 3*log(b2+2) )
Q <- (t(X1)%*%X1)/n1
...
# TODO: Ask Sofia--use average_o1 or take log first? ####
# Is average_o1 really an average of log prices? Based on the magnitude, seems like an average of prices.
# Alternative: Should we be comparing average_o1 to log(pr) instead?
# Log of average not exactly equal to average of logs but closer??
...
round(b5_iv[[3]], 6) == round(b5_2sls[[6]], 6)

# compare ratio and 2SLS coefficient
round(coefratio5, 6) == round(b5_2sls[[6]], 6)
```
