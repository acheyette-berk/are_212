...
*Suppose that there is a model where a structural parameter of interest γ is defined as*
...
G <- cbind(0, (b1_3+3*b1_4)/log(b1_2+2), log(b1_2+2), 3*log(b1_2+2) )
Q <- (t(X1)%*%X1)/n1

vbGammaOLS <- s2* G%*%solve(Q)%*%t(G)
seGammaOLS <- sqrt(diag(vbGammaOLS))
seGammaOLS

# TODO: How to estimate White standard error for gamma??? ####
...
se2 <- sqrt(diag(vb))
```
The `lprice` coefficient from OLS is **`r b2[[3]]`**. This means that for every 100% increase in `price`, `qu` will decrease by about 144%.
...
# TODO: Ask Sofia, estimate eq.1 or eq.2? ####

# define variables X and y and instrument Z
X5 <- cbind(1, my_data$fuel, my_data$lprice, my_data$year, my_data$weight, my_data$luxury)
y5 <- my_data$lqu
Z5 <- cbind(1, my_data$fuel, my_data$laverage_o1, my_data$year, my_data$weight, my_data$luxury)
...
#first stage
Z5_fs <- cbind(1, my_data$fuel, my_data$laverage_o1, my_data$year, my_data$weight, my_data$luxury)
...
#second stage 2SLS
X5_hat <- cbind(1, my_data$fuel, lpricehat, my_data$year, my_data$weight, my_data$luxury)
...
#Xcf <- cbind(1, my_data$lprice, my_data$efs)
Xcf <- cbind(1, my_data$fuel, my_data$lprice, my_data$efs, my_data$year, my_data$weight, my_data$luxury)
...
From Lecture 12: If homoskedasticity then $N R^2 ~ \chi_{Q_1}^2$ where $Q_1 = L_2 - G_2$ and $R^2$ is the R squared of the
regression of the 2SLS residuals on the instruments that is, of the regression $e_{2SLS} = Z \rho + u$
...
#first stage
Z5_fs <- cbind(1, my_data$fuel, my_data$laverage_o1, my_data$laverage_o2, my_data$year, my_data$weight, my_data$luxury)
a_fs <- solve(t(Z5_fs) %*% Z5_fs) %*% t(Z5_fs) %*% my_data$lprice
lpricehat <- Z5_fs %*% a_fs
...
# calculate marginal effects at every observed value of X and average across the resulting effect estimates
...
logit2 <- glm(chosePBM~yourage+treat+relprice+vegpbm, pbm_data, family=binomial(link="logit"))
summary(logit2)
```
...
logit2_rtreat <- glm(chosePBM~yourage+relprice+vegpbm, pbm_data, family=binomial(link="logit"))
...
LRpvalue
```
With a 5% significance level, we would reject the null hypothesis. This means we should use the unrestricted model. This suggests that treatment does matter. 
...
logit2_rvegpbm <- glm(chosePBM~yourage+treat+relprice, pbm_data, family=binomial(link="logit"))
...
With a 5% significance level, we would reject the null hypothesis. This means we should use the unrestricted model. This suggests that being vegetarian and having had PBM before does matter.
...
results6ML <- optim(c(-0.1,-0.1,-0.1),logl.logit,method="BFGS",hessian=T,x=X6ML,y=y6ML)
